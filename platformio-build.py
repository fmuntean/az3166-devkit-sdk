...
FRAMEWORK_VERSION = platform.get_package_version("framework-arduinostm32mxchip")
...
env.Append(
    ...
    CPPDEFINES=[
        ("ARDUINO", 10802),
        ("FRAMEWORK_ARDUINO",FRAMEWORK_VERSION ), # int(FRAMEWORK_VERSION.replace(".", "0"))),
        ("__MBED__", 1),
        ("DEVICE_I2CSLAVE", 1),
        "TARGET_LIKE_MBED",
    ...
    LIBPATH=[
        join(FRAMEWORK_DIR, "system"), # location for libdevkit-sdk-core-lib.a
        #join(FRAMEWORK_DIR, "system", "sdk", "lib"),
        join(FRAMEWORK_DIR, "system", "az3166-driver", "libwlan","TARGET_EMW1062"), #location for libwlan.a
        join(FRAMEWORK_DIR, "system", "az3166-driver"), #libstsafe.a
...
inc_dirs = []
import os;
for d in ("system", join("cores", env.BoardConfig().get("build.core")), "libraries"):
...
env.Prepend(LIBS=["devkit-sdk-core-lib", "m", "wlan", "libstsafe", "stdc++", "gcc"])
#env.Prepend(LIBS=[ "m", "wlan",  "stdc++", "gcc"])
